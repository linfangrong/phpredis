...
{
    static char *kw[] = {"SCAN","SSCAN","HSCAN","ZSCAN","XSCAN"};
    int argc;
    smart_str cmdstr = {0};
...
int redis_xadd_cmd(INTERNAL_FUNCTION_PARAMETERS, RedisSock *redis_sock, char *kw,
    ...
    while (optionidx < argc) {
        ...
        if (!strcasecmp(Z_STRVAL_P(z_args[optionidx]), "finity")) {
            if (argc <= optionidx+1) { efree(z_args); return FAILURE; }
            convert_to_long(z_args[optionidx+1]);
            if (Z_TYPE_P(z_args[optionidx+1])!=IS_LONG || Z_LVAL_P(z_args[optionidx+1])<=0) { efree(z_args); return FAILURE; }
            finity = Z_LVAL_P(z_args[optionidx+1]);
            optionidx += 2;
        } else if (!strcasecmp(Z_STRVAL_P(z_args[optionidx]), "pruning")) {
            if (argc <= optionidx+1) { efree(z_args); return FAILURE; }
            convert_to_string(z_args[optionidx+1]);
            if (!strcasecmp(Z_STRVAL_P(z_args[optionidx+1]), "minscore")) {
                pruning = "MINSCORE";
            } else if (!strcasecmp(Z_STRVAL_P(z_args[optionidx+1]), "maxscore")) {
                pruning = "MAXSCORE";
            } else { efree(z_args); return FAILURE; }
            optionidx += 2;
        } else if (!strcasecmp(Z_STRVAL_P(z_args[optionidx]), "elements")) {
            elements = 1;
            optionidx++;
        } else break;
    }
    ...
    if(argc>0) convert_to_string(z_args[0]);
    if (Z_TYPE_P(z_args[0])!=IS_STRING) { efree(z_args); return FAILURE; }
